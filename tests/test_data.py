...
def test_add_company_get_save_history(df):
    company_id = add_company("test", "forecast_sales_test", "postgres", "egor", "localhost", "5432")
    try:
        save_sales_history(df, company_id, "forecast_sales_test", "postgres", "egor", "localhost", "5432")
        res_df, orig_date = get_sales_history(company_id, "forecast_sales_test", "postgres", "egor", "localhost", "5432")
        ...
        df.loc[1, "price"] = 19
        save_sales_history(df, company_id, "forecast_sales_test", "postgres", "egor", "localhost", "5432")
        res_df, _ = get_sales_history(company_id, "forecast_sales_test", "postgres", "egor", "localhost", "5432")
        res_df = res_df.sort_values(["date", 'product_id']).reset_index(drop=True).drop("company_id", axis = 1)
        pd.testing.assert_frame_equal(df, res_df, check_dtype=False)
    finally:
        with psycopg.connect(
        dbname = "forecast_sales_test",        
        user =  'postgres',          
        password = "egor",   
        host = "localhost",          
        port = "5432" 
...
def test_save_and_get_config():
    company_id = add_company(f"test_config_{uuid4().hex}", "forecast_sales_test", "postgres", "egor", "localhost", "5432")
    try:
        conf_v1 = {
            "updated_at": "2026-08-31T10:00:00+00:00",
            "products": {"P001": {"model": "bl_lag1", "score": 1.0}},
        }
        save_config(company_id, conf_v1, "forecast_sales_test", "postgres", "egor", "localhost", "5432")
        assert get_config(company_id, "forecast_sales_test", "postgres", "egor", "localhost", "5432") == conf_v1

        conf_v2 = {
            "updated_at": "2026-08-31T11:00:00+00:00",
            "products": {"P001": {"model": "bl_lag7", "score": 0.8}},
        }
        save_config(company_id, conf_v2, "forecast_sales_test", "postgres", "egor", "localhost", "5432")
        assert get_config(company_id, "forecast_sales_test", "postgres", "egor", "localhost", "5432") == conf_v2
    finally:
        with psycopg.connect(
            dbname="forecast_sales_test",
            user="postgres",
...
def test_get_sales_history_empty_data():
    ...
    with patch("data.psycopg.connect") as fake_connect:
        ...
        with pytest.raises(CompanyDataNotFoundError):
            get_sales_history(1, "forecast_sales_test", "postgres", "egor", "localhost", "5432")
...
def test_get_config_empty_data():
    ...
    with patch("data.psycopg.connect") as fake_connect:
        ...
        with pytest.raises(ConfigNotFoundError):
            get_config(1, "forecast_sales_test", "postgres", "egor", "localhost", "5432")
